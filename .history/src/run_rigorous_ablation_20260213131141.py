...
class AdapterWrapper:
    ...
    def fit(self, X, y, ite_train=None, egfr_train=None):
        self.model.train()
        ...
        if ite_train is not None and egfr_train is not None:
             dataset = TensorDataset(
                X_ten, y_ten,
                torch.tensor(ite_train, dtype=torch.float32).to(self.device),
                torch.tensor(egfr_train, dtype=torch.float32).to(self.device)
            )
             has_extras = True
        else:
        ...
        for ep in range(1, self.epochs + 1):
            ...
            for batch in loader:
                ...
                loss = loss_base + current_lambda_ite * loss_ite + self.lambda_cate * loss_cate + self.lambda_sens * loss_sens
...
def compute_full_metrics(model, X_test, y_test, key_metrics_extras=None):
    """
...
def main():
    ...
    for seed in seeds:
        ...
        X_train, X_test, y_train, y_test, ite_train, ite_test, egfr_train, egfr_test, feat_names = load_data_with_extras(random_state=seed)
        ...
        model_sota.fit(X_train, y_train, ite_train, egfr_train)
        
        t0 = time.time()
        res_sota = compute_full_metrics(model_sota, X_test, y_test, extras_test)
        ...
        model_v1 = AdapterWrapper(DLCNoHGNN, "w/o HGNN", 
                                epochs=args.wogh_epochs, 
                                lambda_pred=args.wogh_lambda_pred,
                                lambda_hsic=args.wogh_lambda_hsic,
                                lambda_cate=args.wogh_lambda_cate,
                                lambda_ite=args.wogh_lambda_ite,
                                lambda_sens=args.wogh_lambda_sens,
                                sens_eps=args.wogh_sens_eps)
        model_v1.fit(X_train, y_train, ite_train, egfr_train)
        t1 = time.time()
        res_v1 = compute_full_metrics(model_v1, X_test, y_test, extras_test)
        ...
        model_v2.fit(X_train, y_train, ite_train, egfr_train)
        t2 = time.time()
        res_v2 = compute_full_metrics(model_v2, X_test, y_test, extras_test)
        ...
        model_v3 = AdapterWrapper(DLCNet, "w/o HSIC", 
                                ...
                                lambda_cate=args.ablation_lambda_cate,
                                lambda_ite=args.ablation_lambda_ite)
        model_v3.fit(X_train, y_train, ite_train, egfr_train)
        t3 = time.time()
        res_v3 = compute_full_metrics(model_v3, X_test, y_test, extras_test)
