...
def candidate_pool() -> List[Candidate]:
    return [
        Candidate("v6a_acc_guard", v6_alpha_grid="0.0,0.1,0.2,0.3,0.4", v6_acc_floor=0.82),
        Candidate("v6b_balanced", v6_alpha_grid="0.0,0.2,0.4,0.6", v6_acc_floor=0.80),
    ]
...
def build_cmd(c: Candidate) -> List[str]:
    return [
        ...
        "--v5-head-weight", "0.6",
        "--no-v5-head-detach",
        "--v6-enable-adaptive-alpha",
        "--v6-alpha-grid", c.v6_alpha_grid,
        "--v6-acc-floor", str(c.v6_acc_floor),
        "--sota-epochs", "34",
        "--ablation-epochs", "24",
...
def main() -> None:
    ...
    jsonl = LOGS_DIR / f"task19.5_designbook_v6_execution_{run_id}.jsonl"

    rows = []
    for idx, c in enumerate(candidate_pool(), 1):
        tag = f"stage1_{idx:02d}_{c.name}_{run_id}"
        log_path = LOGS_DIR / f"task19.5_designbook_v6_{tag}.log"
        ...
        raw_copy = RESULTS_DIR / f"task19.5_designbook_v6_{tag}_raw.json"
        rpt_copy = REPORTS_DIR / f"task19.5_designbook_v6_{tag}_report.md"
    ...
    stop_reason = "V6 adaptive alpha still did not cross classification boundary" if not any_hit else "V6 adaptive alpha crossed boundary"

    summary = {
        "run_id": run_id,
        "objective": "V6 adaptive-alpha validation on top of decoupled head.",
        "rows": rows,
        "final_best": best,
        "any_hit": any_hit,
    ...
    out_json = RESULTS_DIR / f"task19.5_designbook_v6_execution_{run_id}.json"
    out_md = REPORTS_DIR / f"task19.5_designbook_v6_execution_{run_id}.md"
    out_json.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    lines = [
        "# Task19.5 设计书 V6 执行结果",
        "",
        f"- Run ID: {run_id}",
        f"- 候选数: {len(rows)}",
        f"- any_hit: {any_hit}",
