...
class AdapterWrapper:
    ...
    def __init__(
        ...
        v5_enable_decoupled_head=False,
        v5_head_hidden=32,
        v5_head_alpha=0.35,
        v5_head_weight=0.6,
        v5_head_detach=True,
        v6_enable_adaptive_alpha=False,
        v6_alpha_grid="0.0,0.2,0.4,0.6",
        v6_acc_floor=None,
    ):
        self.device = DEVICE
        ...
        self.v5_enable_decoupled_head = bool(v5_enable_decoupled_head)
        self.v5_head_hidden = int(v5_head_hidden)
        self.v5_head_alpha = float(v5_head_alpha)
        self.v5_head_weight = float(v5_head_weight)
        self.v5_head_detach = bool(v5_head_detach)
        self.v6_enable_adaptive_alpha = bool(v6_enable_adaptive_alpha)
        self.v6_acc_floor = v6_acc_floor
        if isinstance(v6_alpha_grid, str):
            vals = []
            for s in v6_alpha_grid.split(','):
                s = s.strip()
                if not s:
                    continue
                try:
                    ...
                    continue
            self.v6_alpha_grid = vals if vals else [self.v5_head_alpha]
        elif isinstance(v6_alpha_grid, (list, tuple)):
            self.v6_alpha_grid = [float(x) for x in v6_alpha_grid]
        else:
            self.v6_alpha_grid = [self.v5_head_alpha]
        self.best_val_score = None
        self.best_val_epoch = None
        self.best_val_thresh = None
        self.best_blend_alpha = self.v5_head_alpha
        self.cls_head = None
    ...
    def _compute_val_metrics(self, X_val, y_val, ite_val=None, egfr_val=None):
        ...
        adaptive_ready = (
            self.v6_enable_adaptive_alpha
            and self.v5_enable_decoupled_head
        ...
        if not adaptive_ready:
            ...
            val_metrics["BlendAlpha"] = float(self.v5_head_alpha)
            return val_metrics

        old_alpha = float(self.v5_head_alpha)
        best_metrics = None
        best_key = (-1, -1e18)
        for alpha in self.v6_alpha_grid:
            self.v5_head_alpha = float(alpha)
            y_prob = self.predict_proba(X_val)[:, 1]
            val_thresh = select_threshold_from_validation(y_val, y_prob)
            val_metrics = compute_full_metrics(self, X_val, y_val, extras, fixed_threshold=val_thresh)
            val_metrics["BlendAlpha"] = float(alpha)

            score, _ = self._joint_score(val_metrics)
            acc_ok = 1
            if self.v6_acc_floor is not None:
                acc_ok = int(val_metrics["Acc"] >= float(self.v6_acc_floor))
            cur_key = (acc_ok, score)
            if cur_key > best_key:
                best_key = cur_key
                best_metrics = val_metrics

        self.v5_head_alpha = old_alpha
        return best_metrics
    ...
    def fit(
        ...
        for ep in range(1, self.epochs + 1):
            ...
            for batch in loader:
                ...
                if self.v5_enable_decoupled_head and self.cls_head is None:
                    z_dim = int(out['Z_effect'].shape[1])
                    hidden = max(4, self.v5_head_hidden)
                    ...
                    self.cls_optimizer = torch.optim.Adam(self.cls_head.parameters(), lr=1e-3)
                ...
                if self.v5_enable_decoupled_head and self.cls_head is not None:
                    z_effect = out['Z_effect'].detach() if self.v5_head_detach else out['Z_effect']
                    aux_logit = self.cls_head(z_effect)
                    loss_aux = F.binary_cross_entropy_with_logits(aux_logit, by)
            ...
            if self.joint_selection and X_val is not None and y_val is not None:
                if ep >= self.joint_warmup and (ep % self.joint_eval_interval == 0):
                    ...
                    if joint_score > best_score + 1e-9:
                        ...
                        self.best_blend_alpha = float(val_metrics.get("BlendAlpha", self.v5_head_alpha))
                        best_state_dict = copy.deepcopy(self.model.state_dict())
                        best_cls_head_state = copy.deepcopy(self.cls_head.state_dict()) if self.cls_head is not None else None
                        bad_rounds = 0
                    else:
                        bad_rounds += 1
            ...
            if ep % self.log_every == 0:
                print(
                    ...
                    f"aux={loss_aux_sum/steps:.4f}",
                    flush=True
                )

        if best_state_dict is not None:
            self.model.load_state_dict(best_state_dict)
            if best_cls_head_state is not None and self.cls_head is not None:
                self.cls_head.load_state_dict(best_cls_head_state)
            self.v5_head_alpha = float(self.best_blend_alpha)
            ...
            print(
                ...
                f"(score={best_score:.4f}, thr={best_thresh:.2f}, alpha={self.v5_head_alpha:.2f})",
                flush=True
            )
    ...
    def predict_proba(self, X):
        ...
        with torch.no_grad():
            ...
            prob = t * y1 + (1 - t) * y0
            if self.v5_enable_decoupled_head and self.cls_head is not None:
                aux_logit = self.cls_head(out['Z_effect'])
                aux_prob = torch.sigmoid(aux_logit)
                alpha = min(1.0, max(0.0, self.v5_head_alpha))
                prob = (1.0 - alpha) * prob + alpha * aux_prob
...
def main():
    ...
    parser.add_argument("--constraint-auc-min", type=float, default=None)
    parser.add_argument("--constraint-acc-min", type=float, default=None)
    parser.add_argument("--constraint-penalty", type=float, default=200.0)
    parser.add_argument("--pred-boost-start-epoch", type=int, default=None,
                        help="Epoch to start boosting classification weight for Full DLC")
    parser.add_argument("--pred-boost-factor", type=float, default=1.0,
                        help="Multiplicative factor for lambda_pred after pred boost start epoch")
    parser.add_argument("--v5-enable-decoupled-head", action=argparse.BooleanOptionalAction, default=False,
                        help="Enable V5 decoupled classification head on Z_effect for Full DLC")
    parser.add_argument("--v5-head-hidden", type=int, default=32,
                        help="Hidden size of V5 decoupled classification head")
    parser.add_argument("--v5-head-alpha", type=float, default=0.35,
                        help="Blend weight of V5 head probability in predict_proba")
    parser.add_argument("--v5-head-weight", type=float, default=0.6,
                        help="Training loss weight for V5 decoupled head")
    parser.add_argument("--v5-head-detach", action=argparse.BooleanOptionalAction, default=True,
                        help="Detach Z_effect when training V5 decoupled head")
    parser.add_argument("--v6-enable-adaptive-alpha", action=argparse.BooleanOptionalAction, default=False,
                        help="Enable V6 adaptive alpha search on validation for decoupled head")
    parser.add_argument("--v6-alpha-grid", type=str, default="0.0,0.2,0.4,0.6",
                        help="Comma-separated alpha grid for V6 adaptive blend")
    parser.add_argument("--v6-acc-floor", type=float, default=None,
                        help="Optional validation Acc floor in V6 alpha selection")
    ...
    for seed in seeds:
        ...
        # Compute GT Delta CATE for improved comparison (Route B)
        _egfr_mask_gt = egfr_test > 0.5
        gt_delta_cate = float(np.mean(ite_test[_egfr_mask_gt]) - np.mean(ite_test[~_egfr_mask_gt])) if _egfr_mask_gt.any() and (~_egfr_mask_gt).any() else 0.0
        ...
        model_sota = AdapterWrapper(DLCNet, "Full DLC (SOTA)", 
                  epochs=args.sota_epochs, 
                      ...
                      constraint_sens_max=args.constraint_sens_max,
                      constraint_cate_min=args.constraint_cate_min,
                      constraint_auc_min=args.constraint_auc_min,
                      constraint_acc_min=args.constraint_acc_min,
                      constraint_penalty=args.constraint_penalty,
                      fhp_distill_weight=(args.fhp_distill_weight if args.fhp_enable_distill else 0.0),
                      pred_boost_start_epoch=args.pred_boost_start_epoch,
                      pred_boost_factor=args.pred_boost_factor,
                      v5_enable_decoupled_head=args.v5_enable_decoupled_head,
                      v5_head_hidden=args.v5_head_hidden,
                      v5_head_alpha=args.v5_head_alpha,
                      v5_head_weight=args.v5_head_weight,
                      v5_head_detach=args.v5_head_detach,
                      v6_enable_adaptive_alpha=args.v6_enable_adaptive_alpha,
                      v6_alpha_grid=args.v6_alpha_grid,
                      v6_acc_floor=args.v6_acc_floor)
        model_sota.fit(
            X_train,
            y_train,
            ite_train,
        ...
        res_sota["GT_Delta_CATE"] = gt_delta_cate
        ...
        res_v1["Seed"] = seed
        res_v1["GT_Delta_CATE"] = gt_delta_cate
        ...
        res_v2["Seed"] = seed
        res_v2["GT_Delta_CATE"] = gt_delta_cate
        ...
        res_v3["Seed"] = seed
        res_v3["GT_Delta_CATE"] = gt_delta_cate
