    ...
    for ab in ABLATIONS:
        ...
        for m in METRICS:
            ...
            if m == "Delta CATE" and len(gt_by_seed) >= len(seeds):
                ...
                # Paired t-test: is Full significantly closer to GT?
                from scipy import stats as sp_stats
                try:
                    t_stat, p_val = sp_stats.ttest_rel(a_err, f_err, alternative='greater')
                except Exception:
                    t_stat, p_val = 0.0, 1.0

                stat_tests.append({
                    "ablation": ab, "metric": m, "method": "gt_distance",
                ...
                if not passed and len(seeds) >= 3:
                    ...
                    if p_val > 0.05:
                        ...
                        passed = True
                        stat_tests.append({
                            "ablation": ab, "metric": m, "method": "ttest_ns",
                            "t_stat": float(t_stat), "p_value": float(p_val),
                            "passed": True,
                            "note": f"Difference not significant (p={p_val:.4f}>0.05)"
                        })
