...
def plot_sensitivity(csv_file, param_name, display_name, file_prefix, chosen_val):
    ...
    if not chosen_row.empty:
        ax2.scatter(chosen_val, chosen_row['PEHE'], color='red', s=150, zorder=5, edgecolors='black', label='Selected SOTA')
    ...
    if not chosen_row.empty:
        ax2.scatter(chosen_val, chosen_row['AUC'], color='red', s=150, zorder=5, edgecolors='black', label='Selected SOTA')
...
if __name__ == '__main__':
    # Chosen defaults for SOTA (from the sweep script)
    # The default in src/run_parameter_sweep_final.py is 
