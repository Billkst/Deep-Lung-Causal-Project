...
# Import the actual run_single_config logic by dynamically editing or using it directly
from src.run_parameter_sweep_final import run_single_config

def main():
    print(">>> Sweeping HSIC Weight with 128x3...")
    hsic_results = []
    for hsic in [0.0, 0.01, 0.1, 1.0, 10.0]:
        scores = []
        for seed in [42, 43, 44, 45, 46]:
            try:
                # Override to force 128x3
                res = run_single_config(seed, {'lambda_hsic': hsic, 'd_hidden': 128, 'num_layers': 3})
    ...
    print("\n>>> Sweeping CATE Weight with 128x3...", flush=True)
    cate_results = []
    for cate in [0.0, 0.5, 1.0, 2.0, 5.0]:
        scores = []
        for seed in [42, 43, 44, 45, 46]:
            try:
                res = run_single_config(seed, {'lambda_cate': cate, 'd_hidden': 128, 'num_layers': 3})
                print(f"  -> Seed {seed}: AUC={res['AUC']:.4f}, CATE={res['Delta_CATE']:.4f}", flush=True)
                scores.append(res)
            except Exception as e:
                print(f"  -> Seed {seed}: Failed ({e})", flush=True)
